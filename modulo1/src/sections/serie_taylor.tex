...
\dfn{
	Data una funzione $f: A \to \mathbb{R}$ e un punto $x_0 \in 
    \mathcal{D}(A)$, $f(x)$ si dice \textbf{infinitesimo} per $x \to x_0$ se: 
	\begin{equation*}
		\lim_{x \to x_0} f(x) = 0
	\end{equation*}
}
...
Proviamo quindi a trovare la migliore approssimazione di \textbf{grado 0} per 
...
Infatti la costante $b$ è la nostra costante $k$ vista in precedenza. Visto che 
...
Ovviamente quando si parla di \textit{tasso di cambiamento} la lampadina che 
...
Ora che però abbiamo già usato sia un limite che una derivata cosa ci 
...
\end{gather*}
Si comincia a vedere un pattern: infatti il coefficiente angolare del termine 
...
\centering
	...
	\begin{axis}[xmax = 5, xmin = -5, ymax = 4, ymin = -0.5,
		...
		\addplot[domain=-5:5, samples=200]{e^x};
		\addplot[domain=-5:5, samples=200]{1/24*x*x*x*x + 1/6*x*x*x + 1/2*x*x + 
        x + 1};
	\end{axis}
	\end{tikzpicture}
\end{subfigure}
...
\imp{
	La notazione \textbf{\textit{o}-piccolo infatti rappresenta l'errore} che 
    ha la nostra approssimazione. Questo perché un $o(x)$ racchiude tutti i 
    termini che vanno a $0$ più velocemente di $x$, quindi tutti i gradi 
    superiori in quanto polinomio. Infatti un errore $o(x^4)$ racchiude tutti 
    i gradi superiori a $x^4$.
}
...
Ora scegliamo il polinomio di grado 2 che approssima meglio la funzione. 
...
\begin{equation*}
    ...
    x^3 + o(x^3)
\end{equation*}
Che portando tutto a sinistra e applicando la definizione di 
\textit{o}-piccolo:
\begin{equation*}
...
\thm{
	Data una funzione $f:]a,b[ \to \mathbb{R}$ con $0 \in ]a,b[$, se $f$ è 
    derivabile \textit{n}-volte in $x_0 = 0$ si definisce \textbf{polinomio di 
    Taylor} di $f$ in $x_0 = 0$ di grado $\leq n$:
	\begin{equation*}
		T_n(x) = \sum \limits_{i = 0}^n \dfrac{f^{(i)}(0)}{i!} \cdot x^i
	\end{equation*}
	È l'unico polinomio di grado $\leq n$ tale che\footnote{In questo caso non 
    si sa con certezza se il polinomio sarà esattamente di grado $n$, per 
    quello si mette il $\leq$. Questo perché alcune derivate di ordine 
    superiore potrebbero essere nulle facendo quindi risultare il polinomio di 
    grado minore. Si noti però che se comunque il polinomio non risultasse di 
    grado $n$, sarebbe comunque la migliore approssimazione di grado $n$ e 
    avrebbe un \textit{o}-piccolo di $o(x^n)$. Un classico esempio in questo 
    caso sono le funzioni seno e coseno.}: 
	\begin{equation*}
...
\thm{
	Data una funzione $f:]a,b[ \to \mathbb{R}$ con $x_0 \in ]a,b[$, se $f$ è 
    derivabile \textit{n}-volte in $x_0$ si definisce \textbf{polinomio di 
    Taylor} di $f$ in $x_0$ di grado $\leq n$:
	\begin{equation*}
		T_n(x) = \sum \limits_{i = 0}^n \dfrac{f^{(i)}(x_0)}{i!} \cdot 
        (x - x_0)^i
	\end{equation*}
...
\imp{
	È importante notare che il polinomio ci garantisce una buona 
    approssimazione \textbf{solo vicino al punto} $\mathbf{x_0}$. È possibile 
    che la funzione si approssimi anche lontano da quel punto, ma nessuno lo 
    garantisce.
}

\subsection{Sviluppi di Taylor per le funzioni elementari in $x_0 = 0$}
Prima di dare tutti gli sviluppi delle funzioni elementari già calcolati è 
carino vedere come si utilizza effettivamente nella pratica il teorema di 
Peano. Nella sottosezione successiva sono comunque riportati tutti gli sviluppi 
delle funzioni elementari.\\

\textbf{Esponenziale:} Vogliamo calcolare lo sviluppo di Taylor per la funzione 
...
\textbf{Seno:} Calcoliamo lo sviluppo di Taylor per la funzione $f(x) = 
...
\imp{
Una osservazione importante da fare è che nello sviluppo di $\sin{x}$ compaiono 
solo \textbf{termini dispari}. Questo perché la funzione è una funzione 
dispari.
\begin{equation*}
	\sin{(-x)} = -\sin{x}
\end{equation*}
...
Questo è estremamente utile in quanto tutti i termini pari della successione 
...
\textbf{Coseno:} Lo sviluppo della funzione $f(x) = \cos{x} \in C^\infty(
...
\end{equation*}
\imp{
	Come per la funzione seno in cui comparivano solo termini di grado dispari, 
    nella funzione coseno compaio solo \textbf{termini di grado pari}. Questo 
    perché la funzione è una funzione pari.
\begin{equation*}
	\cos{(-x)} = \cos{x}
\end{equation*}
}
...
\textbf{Logaritmo naturale:} Se uno prova a cercare il polinomi di Taylor di 
...
Cominciamo quindi con il calcolare le derivate di $f(x) = \ln{(1 + x)}$ in 
$x = 0$:
\begin{align*}
...
\centering
	...
	\end{tikzpicture}
\end{subfigure}
	\caption{Approssimazioni di grado 1 e 2 della funzione $f(x) = 
    \ln{(1 + x)}$} 
\end{figure}
...
\centering
	...
	\end{tikzpicture}
\end{subfigure}
	\caption{Approssimazioni di grado 3 e 4 della funzione $f(x) = 
    \ln{(1 +x)}$} 
\end{figure}
