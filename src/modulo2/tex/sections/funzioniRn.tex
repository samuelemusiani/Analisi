...
\dfn{
	\textbf{Funzione differenziabile:} Dato $A \subseteq \mathbb{R}^2$ aperto, $(x_0, y_0) \in A$ e $f: A \to \mathbb{R}$ si dice che $f$ è \textbf{differenziabile in $\mathbf{(x_0, y_0)}$} se:
	\begin{enumerate}
		\item $\exists \partial_x f(x_0, y_0), \partial_y f(x_0, y_0) \in \mathbb{R}$
		\item Vale la formula:
			\begin{equation*}
				f(x_0 + h, y_0, + k) = f(x_0, y_0) + \langle \nabla f(x_0, y_0), (h. k)\rangle +o(\lVert (h, k) \rVert) \quad (h, k) \to (0, 0)
			\end{equation*}
	\end{enumerate}
	La 2 si può anche riscrivere come:
	\begin{equation*}
		f(x, y) = f(x_0, y_0) + \langle \nabla f(x_0, y_0), (x - x_0, y - y_0) \rangle + o(\lVert (x - x_0, y - y_0) \lVert) \quad (x, y) \to (x_0, y_0)
	\end{equation*}
}
Ovviamente la notazione con il prodotto euclideo si può espandere:
\begin{align*}
	&\langle \nabla f(x_0, y_0), (h. k)\rangle = \partial_x f(x_0, y_0) \cdot h + \partial_y f(x_0, y_0) \cdot k\\
	&\langle \nabla f(x_0, y_0), (x - x_0, y - y_0) \rangle = \partial_x f(x_0, y_0) \cdot (x - x_0) + \partial_y f(x_0, y_0) \cdot (y - y_0)\\
\end{align*}
\imp{
	Se $f:\mathbb{R}^2 \to \mathbb{R}$ è differenziabile in $(x_0, y_0)$ il polinomio
	\begin{equation*}
		T_1(x, y) = f(x_0, y_0) + \langle \nabla f(x_0, y_0), (x - x_0, y - y_0) \rangle
	\end{equation*}
	si chiama \textbf{polinomio di Taylor di grado 1 di punto inziale $\mathbf{(x_0, y_0)}$}. Inoltre il piano di equazione 
	\begin{equation*}
		z = T_1(x, y)
	\end{equation*}
