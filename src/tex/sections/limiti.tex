...
\mlem{
	È necessario, per questa dimostrazione, introdurre la \textit{disuguaglianza di Bernulli}. 
	\begin{equation*}
		\forall x \in \mathbb{R} : x \geq -1, \forall n \in \mathbb{N}: (1+x)^n \geq 1 + nx
	\end{equation*}
}
\pf{
	Proviamo la disuguaglianza di Bernulli per induzione. 
	\begin{enumerate}
		\item Caso $n = 1$:
			\begin{align*}
		...
		\item Caso $n+1$:
			Ipotesi induttiva sappio che vale $(1+x)^n \geq 1 + nx$.
			Dimostriamo che:
			\begin{align*}
				(1+x)^{n+1} \geq 1 + (n+1)x
			\end{align*}
...
Facciamo ora la dimsotrazione riguardante il numero di Eulero:
...
\begin{enumerate}
    ...
    \end{equation*}
    Dunque $(a_n)_n \nearrow$ è superiormente limitata. Ne consegue che, dal corollario presente nella sezione \ref{corollario_successioni}:
    \begin{equation*}
...
Ne cosegue che graficamente l'asinto verticale si verifca quando la funzione sotto esame tende a $\pm \infty$ in un punto specifico (come in figura \ref{AsintotVer}):
