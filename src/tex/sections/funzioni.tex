...
\dfn{
    Dati due insiemi $A \neq \emptyset$ e $B \neq \emptyset$, una \textbf{funzione} $f$ da $A$ a $B$ ($f: A \to B$) è una corrispondenza univoca da $A$ a $B$, ovvero associa ogni $x \in A$ uno e un solo $y \in B$ (\textbf{legge di associazione}: $\forall x\in A, \exists\oc y\in B : f(x) = y$). Tale elemento è il \textbf{valore della funzione} in $x$ e si scrive $f(x) = y$.\\
...
%AGGIUNGERE IMMAGINI DI FUNZIONI
...
Una funzione, affinché risulti invertibile, è necessario che sia \textbf{biunivoca}.
...
\end{equation*}
Questo permette di togliere l'operazione fattoriale dal primo argomento e quindi permette di generalizzare la sua definzione a $n \in \mathbb{R}$. In questo caso si parla di coefficiente binomiale generalizzato:
\begin{equation*}
	\binom{\alpha}{m} = 
		\begin{cases}
			1 \qquad \qquad \qquad \qquad \qquad \qquad \qquad \,\; \mathrm{se}\;\, m = 0\\[5pt]
		\dfrac{\alpha (\alpha - 1) (\alpha -2) \cdots (\alpha - n + 1)}{n!} \quad \;\, \mathrm{se}\;\, m \geq 1\\
		\end{cases}
\end{equation*}



\subsection{Binomio di Newton} \label{BinomioNewton}
...
\pf{
\textbf{Ipotesi}: $\forall a \in \mathbb{R}_{+}$
    ...
    Prima di procedere con la dimostrazione è necessario prima dimostrare il lemma. Dimostriamo il punto \textbf{A} del lemma:
    \pf{
        \textbf{Ipotesi:} $x^2 \leq y^2$
        \begin{gather*}
            x^2 \leq y^2\\
            x^2 - y^2 \leq 0\\
            (x+y)(x-y) \leq 0
    ...
    La dimostrazione del punto \textbf{B} del lemma è identica al punto \textbf{A} e di conseguenza non verrà riportata. Dimostriamo il punto \textbf{C}:
    \pf{
        \textbf{Ipotesi}: $x^2 = y^2$
        \begin{equation*}
        x^2 = y^2 \iff
            \begin{cases}
    ...
    Dimostriamo ora il punto \textbf{D} del lemma:
    \pf{
        \textbf{Ipotesi}: $x^2 < y$\\
    ...
    \pf{
        ...
        \end{gather*}
        Sempre per il lemma $x > 0$ e per ipotesi $x^2-y > 0$. È quindi possibile scegliere un $\epsilon$ affinché la tesi si verifichi:
        \begin{equation*}
...
\dfn{
Dato un angolo $t$ formato da un raggio sulla circonferenza goniometrica e indicato con $P(x_p, y_p)$ l'estremo del raggio che non è il centro della circonferenza, definiamo:
\begin{gather*}
    \sin{t} \vcentcolon = y_p\\
    \cos{t} \vcentcolon = x_p
\end{gather*}
}
...
Oltre al seno e al coseno esiste un'ulteriore funzione goniometrica chiamata \textbf{tangente}. La definizione geometrica della tangente richiede l'introduzione di una retta di equazione $x = 1$. Una retta quindi verticale tangente alla circonferenza nel punto $(1, 0)$. Se proviamo ora a prendere un qualsiasi punto su questa retta e ha tracciare un segmento che collega il punto al centro della circonferenza delimitiamo un angolo. La tangente di questo angolo sarà l'ordinata del punto. %AGGIUGERE FOTO

\dfn{
    Preso un punto $Q(1, y_q)$ qualsiasi sulla retta $x = 1$ tangente alla circonferenza goniometrica in $(1,0)$, tracciando il segmento che ha come estremi il punto e il centro della circonferenza si trova un angolo $\theta$. La \textit{tangente} di quell'angolo è l'ordinata del punto $Q$:
    \begin{equation*}
        \tan{\theta} \vcentcolon = y_q
    \end{equation*}
}
...
% DA FARE CON UN EDITOR INTEGRATO 
...
Di seguito le formule di \textbf{addizione} e \textbf{sottrazione}: \label{FormuleAddSott}
\begin{gather*}
...
Formule di \textbf{duplicazione}:\label{formuleDuplicazione}
\begin{align*}
...
\end{center}}
Il numero $e$ è descritto nella sezione relativa al numero di Eulero (Sezione \ref{NumeroDiEulero}) e si dice che l'esponenziale in questa base è "naturale" perché ha moltissime proprietà che vedremo in seguito.
...
\end{axis}
\end{tikzpicture}
\caption{Grafico di $y= a^x$ con $a > 1$}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Grafico di $y= b^x$ con $0 < b < 1$}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Grafico di $y= \log_a(x)$ con $ a > 1$}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Grafico di $y= \log_b(x)$ con $0 < b < 1$}
\end{figure}
