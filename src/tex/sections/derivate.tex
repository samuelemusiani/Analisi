...
\begin{itemize}
	...
	\begin{axis}[xmax = 6, xmin = -6, ymax = 1.5, ymin = -1.3,
		...
		\addplot[domain=-6:-0.02, samples=100]{2 * x * sin(deg(1/x)) - cos(deg(1/x))};
		\addplot[domain=0.02:6, samples=100]{2 * x * sin(deg(1/x)) - cos(deg(1/x))};
	\end{axis}
	\end{tikzpicture}
		\caption{Grafico di $2x \cdot \sin(\dfrac{1}{x}) - \cos(\dfrac{1}{x})$ AUMENTARE SAMPLES AL GRAFICO}
...
% Il confronto di infiniti l'ho già riportato in maniera approssimativa nella parte dei limiti. Io lascerei così e rimanderei e una eventuale sezione qui per la dimostrazione perché serve De L'Hopital. Non sposterei il confronto tutto qui perché ha senso che stia con i limiti 
